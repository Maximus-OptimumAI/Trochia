...
1. **FOLLOWUP-NODE-VERSION-SKEW-01 — CLOSED 2026-06-03.** Aligned everything to Node **24.x** (least-change upward; CI `ci.yml`+`eval.yml` were already `node-version: 24`, Vercel default + LTS is 24, local toolchain is v24.15.0). Added `package.json` `engines.node = "24.x"` (was absent — the actual skew was an unpinned `engines`, not a 22-vs-24 CI mismatch). `next` requires `>=20.9.0`; no dependency requires a non-24 version. Voyage adapter MSW tests pass on the v24 toolchain (full vitest green). Recorded in `tasks/constraints.md` (Toolchain / runtime). (`@types/node` left at `^20` — type defs only; bump deferred.)
2. **FOLLOWUP-SOURCEMAP-PROD-PUBLIC-HARDENING-01 — CLOSED 2026-06-03 (leak-guard only).** The T05 wording assumed `withSentryConfig` `hideSourceMaps:true` existed — it did NOT: `withSentryConfig` was stripped by 404-debug commit `16f29b2` and never restored, and neither `productionBrowserSourceMaps` nor `hideSourceMaps` ever appeared in `next.config.ts` history. Founder-approved minimal guard applied: `next.config.ts` now sets `productionBrowserSourceMaps: false`, and `scripts/assert-no-public-sourcemaps.mjs` runs in `postbuild` and FAILS the build if any `.map` appears under `.next/static/chunks/`. Verified empirically: a real `npm run build` produced **zero** `.map` under `.next/static/` and the postbuild assertion printed PASS. `withSentryConfig` was deliberately NOT restored — split out to FOLLOWUP-SENTRY-BUILD-INTEGRATION-RESTORE-01 below.
...
4. **FOLLOWUP-COST-RATES-RATIFY — CLOSED 2026-06-03.** Verified `rates.ts` against current published Anthropic Opus + Voyage pricing (platform.claude.com/docs/en/about-claude/pricing, pulled 2026-06-03). Found the Opus constants carried the DEPRECATED Opus 4 / 4.1 card ($15 / $75 / $18.75 / $1.50) — exactly 3× the current Opus 4.6/4.7/4.8 rates; the live `reason` model is Opus 4.7, so the cap metered real spend at 3× (the "$5/user/day" HARD-blocked at ~$1.67/day real). The internal `actual ≤ reserve` bound held throughout (same constants both sides; ordering + byte-length ceiling intact even under the +35% Opus-4.7 tokenizer), so it was conservative-but-wrong, not unsafe. Corrected (founder-approved, values-only, no cap-enforcement/wiring change): OPUS_INPUT=5.00 (single source of truth), OPUS_OUTPUT=25.00, OPUS_CACHE_WRITE=input×1.25=6.25, OPUS_CACHE_READ=input×0.10=0.50; Voyage 0.18 unchanged (already correct). `rates.test.ts` now pins the ABSOLUTE input rate + the two derived values so a future absolute-rate drift fails CI (the prior ordering/proportion-only checks let the 3× error sit invisible). Commit cites FOLLOWUP-COST-RATES-RATIFY.
...
- **FOLLOWUP-SENTRY-BUILD-INTEGRATION-RESTORE-01** (OPEN — surfaced 2026-06-03 during the sourcemap-hardening gate) — `next.config.ts`'s `withSentryConfig` wrapper is GONE. Timeline: `312d31f feat(01-05)` wired it (build-time source-map upload via `SENTRY_AUTH_TOKEN`, release tagging, `tunnelRoute: "/monitoring"`, `widenClientFileUpload`, `disableLogger`); `16f29b2 test: strip withSentryConfig to isolate 404 source` (2026-05-15) deleted the whole wrapper; the 404 hunt then moved to other suspects (`58a0176` proxy.ts rename → `4420d7b` reverted, "not the 404 source") and **the wrapper was never restored (~3 weeks).** **Currently OFF in prod:** build-time source-map upload to Sentry, release/commit tagging, and the `/monitoring` tunnel. **Runtime capture status (item-4 finding):** SERVER + EDGE error capture is **INTACT** — `instrumentation.ts` `register()` (native Next 16 hook) loads `sentry.server/edge.config.ts` + `onRequestError`, independent of the build plugin. BROWSER/CLIENT capture is **NOT ACTIVE** — `sentry.client.config.ts` is auto-injected into the webpack entry **by the Sentry build plugin** (`@sentry/nextjs` `webpack.js:418`), which is stripped; there is **no `instrumentation-client.ts`** (the Turbopack-safe successor Next 16 loads natively), so browser `Sentry.init` never runs → **client-side errors are invisible in prod at debut.** Restore requires: (a) decide whether `withSentryConfig` caused the original 404 (restore-and-test, do NOT trust blindly); (b) move client init to `instrumentation-client.ts` (the deprecation path — `sentry.client.config.ts` won't work under Turbopack, which Next 16 builds default to); (c) re-decide the `/monitoring` `tunnelRoute` (a routing change). Until restored, the leak-guard (FOLLOWUP-SOURCEMAP-PROD-PUBLIC-HARDENING-01) holds independently via `productionBrowserSourceMaps:false` + the postbuild assertion. **NOT implemented — founder-gated.**

## Carried-forward follow-ups (unchanged)

PULL-OBS-COST-01-FORWARD (DELIVERED here; only SEC-02 tier-aware caps carry to Phase 4.5), FOLLOWUP-DBDIFF-01 (ACTIVE), FOLLOWUP-COST-RESERVATION-RECLAIM-01 (P2-C crash-strand sweeper), FOLLOWUP-QA-PERSISTENCE-AUDIT-OPT-IN (OD-5 opt-in full-query persist), FOLLOWUP-QA-GROUNDING-FIXTURE-EXPAND (full 50+10 Q-set), FOLLOWUP-QA-STREAMING-01 (OD-6 true token-streaming), ~~FOLLOWUP-COST-RATES-RATIFY~~ (CLOSED 2026-06-03 — Opus rates corrected 3× → published 4.6/4.7/4.8; absolute-rate test pinned), FOLLOWUP-SANITIZER-EVAL-01, FOLLOWUP-FTS-GIN-INDEX-01, FOLLOWUP-HARDCODED-DOMAIN-REGEX-01 (the 2 known vitest failures — D4 carve-out, route through CCO before tightening).
